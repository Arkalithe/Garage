<?php
include_once 'Connect.php';

class AddDataAvis {
    public function dataAvis(){
        $db_connection = new DatabaseConnect();
        $conn = $db_connection->dbConnectionNamed();
        try {
            $avis = [
                ['name' => 'John Marchand', 'message' => 'Content du service', 'note' => '4', 'moderate' =>  0],
                ['name' => 'Doe Inconnu', 'message' => 'Prix abordable et personelle agrable',  'note' => '5', 'moderate' =>  0],
                ['name' => 'Angry User', 'message' => 'Ils ont prit leurs temps alors que j"etais pressé', 'note' => '2', 'moderate' => 0]
            ];

            $stmt = $conn->prepare("INSERT INTO avis (name, message, note)
                VALUES (?, ?, ?)");

            foreach ($avis as $data) {
                $stmt->execute([$data['name'], $data['message'], $data['note']]);
                echo 'Horaire ajouté .<br>';
            }
            echo 'Avis ajouté .<br>';
        }
        catch(PDOException $e) {
            
        }

    }
}